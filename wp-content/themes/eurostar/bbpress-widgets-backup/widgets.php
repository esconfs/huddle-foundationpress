		...
		<ul>
			...
			<?php while ( $widget_query->have_posts() ) :
				...
				if ( ! empty( $settings['show_user'] ) ) :
					$author_link = bbp_get_topic_author_link( array( 'post_id' => $topic_id, 'type' => 'name', 'size' => 14 ) );
					$author_img = bbp_get_topic_author_link( array( 'post_id' => $topic_id, 'type' => 'avatar', 'size' => 30 ) );
				endif; ?>
				...
				<li>
					...
					<?php if ( ! empty( $author_link ) ) : ?>
						...
						<?php printf( '<span class="topic-author">' . $author_link . '</span>' ); ?>
					...
					<?php if ( ! empty( $settings['show_date'] ) ) : ?>
						...
						<div class="bbp-forum-time"><?php bbp_topic_last_active_time( $topic_id ); ?></div>
		...
		<ul>
			...
			<?php while ( $widget_query->have_posts() ) : $widget_query->the_post(); ?>
				...
				<li>
					...
					if ( ! empty( $settings['show_user'] ) ) :
						$author_link = bbp_get_reply_author_link( array( 'post_id' => $reply_id, 'type' => 'name', 'size' => 14 ) );
						$author_img = bbp_get_reply_author_link( array( 'post_id' => $reply_id, 'type' => 'avatar', 'size' => 30 ) );
					else :
						$author_link = false;
						$author_img = false;
					endif;
					...
					if ( ! empty( $settings['show_date'] ) && !empty( $author_link ) ) :
						...
						echo '<div class="bbp-forum-time">' . bbp_get_time_since( get_the_time( 'U' ) ) . '</div>';
						
						// translators: 1: reply author, 2: reply link, 3: reply timestamp
						// printf( _x( '%1$s on %2$s %3$s', 'widgets', 'bbpress' ), $author_link, $reply_link, '<div class="bbp-forum-time">' . bbp_get_time_since( get_the_time( 'U' ) ) . '</div>' );
