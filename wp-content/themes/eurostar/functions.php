...
function filter_plugin_updates( $value ) {
    //unset( $value->response['custom-post-type-permalinks/custom-post-type-permalinks.php'] );
...
add_image_size( 'forum_header_advert', 650, 90 );
add_image_size( 'paid_sidebar_advert', 300, 150 );
add_image_size( 'banner-image', 750, 330, true );
add_image_size( 'related-advert', 300, 250, true );
add_image_size( 'slider_image', 315, 195, true );
...
add_filter('pre_get_posts', function( $query ) {
    if (!$query->is_admin && $query->is_search && $query->is_main_query() ) {
        ...
        $query->set('post_parent__not_in', array(15841));
        $query->set('post__not_in', array(879, 120, 268, 105,13787, 11268, 15245, 13466, 14419, 11755, 13607, 13681, 12491, 14533, 13969, 11374, 11319, 13920, 13629, 15715, 15399, 15007, 13449, 12026, 11459, 14544, 13129, 16119, 16107, 16112, 15950));
    }
    return $query;
});
...
function custom_breadcrumbs() {
	   ...
	   //Rescource
	   echo   $crumbs[1] . " / " . $end_url;
	   //URL SECTION
	   echo $open_url . "/" .$crumbs[1] . "/" . $crumbs[2] ."/"  . $close_url;
		//Category
		echo    $crumbs[2] . " / " .  $end_url;
	   //URL SECTION
...
if ( $taxonomy == 'resource_type' || 'resource_type-sitemap' ) return true;
}
