...
$image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'saxon-blog-thumb-masonry');
...
echo '<div class="saxon-grid-info-post saxon-masonry-post saxon-post'.esc_attr($post_class).'"'.saxon_add_aos(false).'>';

if(has_post_thumbnail( $post->ID )) {
  echo '<div class="saxon-post-image-wrapper"><a href="'.esc_url(get_permalink($post->ID)).'"><div class="saxon-post-image"><img src="'.esc_attr($image[0]).'" alt="'.esc_attr($post->post_title).'"/>'.wp_kses_post($post_format_icon).'</div></a><div class="post-categories">'.wp_kses($categories_list, saxon_esc_data()).'</div></div>';
} else {
...
<?php echo '<div class="post-excerpt">'.wp_kses_post( saxon_get_the_excerpt(330, $post->ID) ).'</div>'; ?>
<?php endif; ?>
...
<div class="post-info-comments"><i class="fa fa-comment-o" aria-hidden="true"></i><a href="<?php echo esc_url(get_comments_link( $post->ID )); ?>"><?php echo esc_html(get_comments_number_text( esc_html__('0', 'saxon'), esc_html__('1', 'saxon'), esc_html__('%', 'saxon') )); ?></a></div>
<?php endif; ?>
<?php endif; ?>
