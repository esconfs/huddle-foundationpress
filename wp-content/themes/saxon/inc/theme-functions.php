...
function saxon_logo_display() {
    ...
    // Image logo
        ...
        ?>
        <div class="logo">
        <a class="logo-link" href="<?php echo  esc_url(home_url('/')); ?>"><img src="<?php echo get_header_image(); ?>" alt="<?php bloginfo('name'); ?>" class="regular-logo"><img src="<?php if ( get_theme_mod( 'saxon_header_transparent_logo' ) ) { echo esc_url( get_theme_mod( 'saxon_header_transparent_logo' )); } else { echo esc_url(get_header_image()); }  ?>" alt="<?php bloginfo('name'); ?>" class="light-logo"></a>
        <?php
          if(get_bloginfo('description') !== '' && get_theme_mod( 'header_tagline', false ) ) {
            echo '<div class="header-blog-info header-blog-info-'.esc_attr($tag_line_style).'">';
            bloginfo('description');
...
function saxon_topline_display() {
  ...
  if(get_theme_mod('header_topline', false)) {
    ...
    if(get_theme_mod('header_topline_button_title', '') !== '') {
      ...
      $header_topline_button_html = '<div class="header-topline-button"><a href="'.get_theme_mod('header_topline_button_url', '').'" class="btn btn-small btn-transparent"'.esc_html($button_target).'>'.esc_html(get_theme_mod('header_topline_button_title', '')).'</a></div>';
    } else {
...
function saxon_header_right_display() {
    ...
    ?>
    <div class="search-toggle-wrapper search-<?php esc_attr_e(get_theme_mod('search_position', 'header')); ?>">
...
function saxon_loadmore_ajax_handler(){
  ...
  if( have_posts() ) :
    ...
    while( have_posts() ): the_post();
      ...
      // Posts grid layouts
      // Also set in saxon_posts_block_renderer()

      $col_class = 'col-md-12';

      if($block_id == 'postsgrid1') {
